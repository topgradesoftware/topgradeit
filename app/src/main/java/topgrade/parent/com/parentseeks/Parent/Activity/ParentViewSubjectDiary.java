...
public class ParentViewSubjectDiary extends AppCompatActivity {

    private static final String TAG = "ParentViewSubjectDiary";
    ...
    private SharedStudent selectedStudent = null; // Store selected student to access student name
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_parent_view_subject_diary);
        ...
        diaryRecyclerView = findViewById(R.id.diary_rcv);
                ...
                select_child_spinner.setOnItemClickListener((parent, view, position, id) -> {
                    if (position >= 0 && position < student_name_list.size()) {
                        ...
                        if (selectedStudent != null) {
                            ...
                            ParentViewSubjectDiary.this.selectedStudent = selectedStudent; // Store selected student
    ...
    private void loadSubjectDiary() {
        ...
        Constant.mApiService.load_diary(body).enqueue(new Callback<Diary>() {
                ...
                if (response.isSuccessful()) {
                    ...
                    if (diary != null && diary.getStatus() != null) {
                        ...
                        if (isSuccess) {
                            ...
                            List<Object> dataList = diary.getData();
                            
                            if (dataList != null && !dataList.isEmpty()) {
                                try {
                                    Gson gson = new Gson();
                                    JsonElement jsonElement = gson.toJsonTree(dataList.get(0));
                                    JsonObject dataObject = jsonElement.getAsJsonObject();
                                    
                                    // Extract date from parent object (full_name contains the date)
                                    String date = dataObject.has("full_name") ? 
                                        dataObject.get("full_name").getAsString() : null;
                                    
                                    // Extract class name from parent object
                                    ...
                                    // Only show entries that have subject_name (subject diary entries)
                                    diaryList.clear();
                                    for (int i = 0; i < 20; i++) { // Check up to 20 subjects
                                        ...
                                        if (dataObject.has(key)) {
                                            ...
                                            // Only add if it has a subject name (subject diary entry, not class diary)
                                            if (subjectEntry != null && subjectEntry.getSubjectName() != null && 
                                                !subjectEntry.getSubjectName().isEmpty()) {
                                                // Set date and class name from parent object
                                                if (date != null) {
                                                    subjectEntry.setDate(date);
                                                }
                                                if (className != null) {
                                                    subjectEntry.setClassName(className);
                                                }
                                                // Set actual student name, not date
                                                if (studentFullName != null) {
                                                    subjectEntry.setFullName(studentFullName);
                                                    subjectEntry.setStudentName(studentFullName);
                                                }
                                                
                                                // Extract image URL from subject object
                                                String imageUrl = null;
                                                
                                                // Check various possible image field names in subject object
                                                if (subjectObj.has("image_url") && !subjectObj.get("image_url").isJsonNull()) {
                                                    imageUrl = subjectObj.get("image_url").getAsString();
                                                    Log.d(TAG, "Found image_url in subject " + key + ": " + imageUrl);
                                                } else if (subjectObj.has("picture") && !subjectObj.get("picture").isJsonNull()) {
                                                    imageUrl = subjectObj.get("picture").getAsString();
                                                    Log.d(TAG, "Found picture in subject " + key + ": " + imageUrl);
                                                } else if (subjectObj.has("image") && !subjectObj.get("image").isJsonNull()) {
                                                    imageUrl = subjectObj.get("image").getAsString();
                                                    Log.d(TAG, "Found image in subject " + key + ": " + imageUrl);
                                                } else if (subjectObj.has("diary_image") && !subjectObj.get("diary_image").isJsonNull()) {
                                                    imageUrl = subjectObj.get("diary_image").getAsString();
                                                    Log.d(TAG, "Found diary_image in subject " + key + ": " + imageUrl);
                                                }
                                                
                                                // If no image in subject object, try parent object
                                                ...
                                                if (imageUrl != null && !imageUrl.isEmpty()) {
                                                    subjectEntry.setImageUrl(imageUrl);
                                                    subjectEntry.setPicture(imageUrl);
                                                    Log.d(TAG, "Set image URL in subject entry: " + imageUrl);
                                                } else {
                                                    Log.w(TAG, "No image URL found for subject " + key);
                                                }
                                                
                                                diaryList.add(subjectEntry);
                                            }
                                        }
                                    }
                                    ...
                                    if (diaryList.isEmpty()) {
                                        Toast.makeText(context, "No subject diary entries found", Toast.LENGTH_SHORT).show();
                                    }
                                } catch (Exception e) {
                                    Log.e(TAG, "Error parsing subject diary data", e);
                                    Toast.makeText(context, "Error parsing diary data.", Toast.LENGTH_SHORT).show();
                                }
                            } else {
                                diaryList.clear();
                                adapter.notifyDataSetChanged();
                                Toast.makeText(context, "No diary entries found for the selected date.", Toast.LENGTH_SHORT).show();
                            }
                        } else {
            ...
            public void onFailure(@NonNull Call<Diary> call, @NonNull Throwable t) {
                ...
                Log.e(TAG, "Error loading subject diary", t);
