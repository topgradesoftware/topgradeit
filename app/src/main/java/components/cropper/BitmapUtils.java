...
final class BitmapUtils {
  ...
  static Uri writeTempStateStoreBitmap(Context context, Bitmap bitmap, Uri uri) {
    try {
      ...
      } else if (uri.getPath() != null && new File(uri.getPath()).exists()) {
        needSave = false;
      }
      if (needSave) {
        // Use JPEG format with 95% quality for temporary state storage
        writeBitmapToUriJPEG(context, bitmap, uri);
      }
      return uri;
    } catch (Exception e) {
  ...
  /** Write the given bitmap to the given uri using JPEG compression with 95% quality. */
  private static void writeBitmapToUriJPEG(Context context, Bitmap bitmap, Uri uri)
  ...
  private static Bitmap decodeImage(
    ...
    // Use modern ImageDecoder for Android P+ (API 28+)
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
      try {
        ImageDecoder.Source source = ImageDecoder.createSource(resolver, uri);
        final int targetSampleSize = options.inSampleSize;
        return ImageDecoder.decodeBitmap(source, (decoder, info, src) -> {
          // Set target size based on sample size
          if (targetSampleSize > 1) {
            int targetWidth = info.getSize().getWidth() / targetSampleSize;
            int targetHeight = info.getSize().getHeight() / targetSampleSize;
        ...
        throw new RuntimeException("Failed to decode image with ImageDecoder: " + uri, e);
      } catch (OutOfMemoryError e) {
        // Fallback to BitmapFactory with increased sample size
        options.inSampleSize *= 2;
        if (options.inSampleSize > 512) {
          throw new RuntimeException("Failed to decode image: OOM even with max sampling", e);
        }
        // Retry with legacy method
        return decodeImageLegacy(resolver, uri, options);
      }
    }
  ...
  /**
   * Legacy bitmap decoding using BitmapFactory for Android versions below P.
   * Also used as fallback when ImageDecoder fails.
   */
  private static Bitmap decodeImageLegacy(
      ContentResolver resolver, Uri uri, BitmapFactory.Options options)
      throws FileNotFoundException {
    do {
      InputStream stream = null;
    ...
    try {
      ...
      if (stream != null) {
        ...
        if (decoder != null) {
          do {
            try {
              Bitmap decodedBitmap = decoder.decodeRegion(rect, options);
              if (decodedBitmap != null) {
                return new BitmapSampled(decodedBitmap, options.inSampleSize);
              }
            } catch (OutOfMemoryError e) {
              options.inSampleSize *= 2;
            }
   ...
   * <a href="http://stackoverflow.com/questions/7428996/hw-accelerated-activity-how-to-get-opengl-texture-size-limit">Stack Overflow Reference</a>
   */
   ...
   * @param sampleSize The sample size used to lower the size of the bitmap (1,2,4,8,...)
   */
  record BitmapSampled(Bitmap bitmap, int sampleSize) {}
  // endregion
   ...
   * @param degrees The degrees the image was rotated
   */
  record RotateBitmapResult(Bitmap bitmap, int degrees) {}
  // endregion
}
