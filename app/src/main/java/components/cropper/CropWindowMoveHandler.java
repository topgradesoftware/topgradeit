...
final class CropWindowMoveHandler {
  ...
  /** Damping factor for edge resistance when dragging beyond bounds */
  private static final float EDGE_RESISTANCE_DAMPING = 1.05f;

  /** Touch offset adjustment factor for smoother dragging */
  private static final float TOUCH_OFFSET_ADJUSTMENT = 1.1f;

  /** Divisor for boundary offset calculations */
  private static final float BOUNDARY_OFFSET_DIVISOR = 2f;
   ...
   *
   * @param rect the crop window rectangle to modify
   ...
   * @param fixedAspectRatio is the aspect ratio fixed and 'aspectRatio' should be used
  ...
  private void moveCenter(
    ...
    if (rect.left + dx < 0
      ...
      dx /= EDGE_RESISTANCE_DAMPING;
      mTouchOffset.x -= dx / BOUNDARY_OFFSET_DIVISOR;
    }
    if (rect.top + dy < 0
        || rect.bottom + dy > viewHeight
        || rect.top + dy < bounds.top
        || rect.bottom + dy > bounds.bottom) {
      dy /= EDGE_RESISTANCE_DAMPING;
      mTouchOffset.y -= dy / BOUNDARY_OFFSET_DIVISOR;
    }
    rect.offset(dx, dy);
  ...
  private void adjustLeft(
    ...
    if (newLeft < 0) {
      newLeft /= EDGE_RESISTANCE_DAMPING;
      mTouchOffset.x -= newLeft / TOUCH_OFFSET_ADJUSTMENT;
    }

    if (newLeft < bounds.left) {
      mTouchOffset.x -= (newLeft - bounds.left) / BOUNDARY_OFFSET_DIVISOR;
    }
  ...
  private void adjustRight(
    ...
    if (newRight > viewWidth) {
      newRight = viewWidth + (newRight - viewWidth) / EDGE_RESISTANCE_DAMPING;
      mTouchOffset.x -= (newRight - viewWidth) / TOUCH_OFFSET_ADJUSTMENT;
    }

    if (newRight > bounds.right) {
      mTouchOffset.x -= (newRight - bounds.right) / BOUNDARY_OFFSET_DIVISOR;
    }
  ...
  private void adjustTop(
    ...
    if (newTop < 0) {
      newTop /= EDGE_RESISTANCE_DAMPING;
      mTouchOffset.y -= newTop / TOUCH_OFFSET_ADJUSTMENT;
    }

    if (newTop < bounds.top) {
      mTouchOffset.y -= (newTop - bounds.top) / BOUNDARY_OFFSET_DIVISOR;
    }
  ...
  private void adjustBottom(
    ...
    if (newBottom > viewHeight) {
      newBottom = viewHeight + (newBottom - viewHeight) / EDGE_RESISTANCE_DAMPING;
      mTouchOffset.y -= (newBottom - viewHeight) / TOUCH_OFFSET_ADJUSTMENT;
    }

    if (newBottom > bounds.bottom) {
      mTouchOffset.y -= (newBottom - bounds.bottom) / BOUNDARY_OFFSET_DIVISOR;
    }
   ...
   * remains in position while the right adjusts to keep aspect ratio to the height.
   */
   ...
   * remains in position while the bottom adjusts to keep aspect ratio to the width.
   */
