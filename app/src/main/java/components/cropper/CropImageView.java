...
public class CropImageView extends FrameLayout {
  ...
  public CropImageView(Context context, AttributeSet attrs) {
    ...
    if (options == null) {
      ...
      if (attrs != null) {
        try (TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.CropImageView, 0, 0)) {
          options.isFixAspectRatio =
              ta.getBoolean(R.styleable.CropImageView_cropFixAspectRatio, options.isFixAspectRatio);
          ...
          options.isAutoZoomEnabled =
              ta.getBoolean(R.styleable.CropImageView_cropAutoZoomEnabled, options.isAutoZoomEnabled);
          options.isMultiTouchEnabled =
              ta.getBoolean(
                  R.styleable.CropImageView_cropMultiTouchEnabled, options.isMultiTouchEnabled);
          ...
          options.maxCropResultHeight =
              (int)
                  ta.getFloat(
                      ...
                      options.maxCropResultHeight);
          options.isFlipHorizontally =
              ta.getBoolean(
                  R.styleable.CropImageView_cropFlipHorizontally, options.isFlipHorizontally);
          options.isFlipVertically =
              ta.getBoolean(R.styleable.CropImageView_cropFlipHorizontally, options.isFlipVertically);
    ...
    if (mBitmap != null) {
      ...
      double change =
          Math.sqrt(
              Math.pow(BitmapUtils.POINTS2[4] - BitmapUtils.POINTS2[2], 2)
                  + Math.pow(BitmapUtils.POINTS2[5] - BitmapUtils.POINTS2[3], 2));
      halfWidth *= (float) change;
      halfHeight *= (float) change;
  ...
  protected void onDetachedFromWindow() {
    ...
    // Cancel any running background tasks to prevent memory leaks
    if (mBitmapLoadingWorkerTask != null) {
      BitmapLoadingWorkerTask task = mBitmapLoadingWorkerTask.get();
      if (task != null) {
        task.cancel();
    ...
    if (mBitmapCroppingWorkerTask != null) {
      BitmapCroppingWorkerTask task = mBitmapCroppingWorkerTask.get();
      if (task != null) {
        task.cancel();
      }
      mBitmapCroppingWorkerTask = null;
    ...
    // Clear animation to prevent leaks
    if (mAnimation != null) {
      clearAnimation();
      mAnimation = null;
    }
  ...
  // Add public getters for loaded image dimensions
